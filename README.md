...
VCS 全库检查只在桌面端设置窗口提供,不暴露公网 REST 入口。检索 / 读取 / 上传文档等不受此限。公网暴露请自行在前置网关加鉴权。

## MCP 接入(Claude / Codex)

Ktree 在 `http://<host>:<port>/mcp` 暴露 Streamable HTTP MCP Server。该端点只接受 JSON-RPC `POST`,浏览器 `GET` 返回 `405 Method Not Allowed` 是正常现象。AI 客户端接入后即可直接检索、读取和写入知识库:
...
最小读取流程:`kb_search` 搜索 → `kb_get_doc(id)` 取全文 → 需要目录上下文时用 `kb_list_docs`。MCP 工具里的 `path` 使用文档相对 `src/` 的路径,例如 `vcs/svn/策划文档`;静态直链才写成 `/<知识库>/src/...` 或 `/<知识库>/docs/...`。

显式忽略规则:任一路径层级的目录名或文件名以 `##!` 或 `##！` 开头时,Ktree 不自动转换、不加入索引,AI 工具也不会返回。其它目录名不做语义判断,例如“归档”“AI 不看”本身不会被特殊处理。
...
如需让某个仓库目录或文件不进入知识库,把该层级名称改成 `##!名称` 或 `##！名称`。该规则按路径层级统一生效,只表示“不转换、不索引”;不会因为目录名包含“归档”“AI 不看”等文字而自动跳过。
